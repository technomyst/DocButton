...
[2. Внешний вид](#7-дополнительные-рекомендации-и-полезные-сведения)

[3. Состояния](#3-состояния)

[4. Размеры](#5-рекомендации-по-формулировкам-и-содержанию)

[5. Рекомендации по формулировкам и содержанию](#7-дополнительные-рекомендации-и-полезные-сведения)
...
| `tertiary`          | Минимально выраженная кнопка. Подходит для дополнительных, не критичных действий. |
...
| **Loading**    | Кнопка в процессе выполнения действия. |
...
| `medium`         | Основной размер по умолчанию. Рекомендуется для большинства случаев. |
| `small`          | В компактных интерфейсах: таблицах, фильтрах, списках. |
...
- Уточнить про параметры `size` и `vertSize`. Можно ли указать только число, обозначающее пиксели? Или возможно указание размера в `%` и других единица?.
